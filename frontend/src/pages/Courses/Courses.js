...
const Courses = () => {
  ...
  const types = ["Live", "Free", "Self-Paced", "Classroom", "All"];
  const options = ["All", "Web Development", "DS & Algorithms", "Programming Languages", "Machine Learning","Interview & Placement", "Data Science"]
  ...
  const handleSearch = async () => {
    try {
      const response = await fetch(
        `http://localhost:5002/courses/search?q=${searchTerm}&category=${category}`
      );
  ...
  const handleCategoryWiseSearch = async (category) => {
    setCategory(category);
    setType("All");
    console.log(type)
    try {
      const response = await fetch(
        `http://localhost:5002/courses/category/search/${category}`
      );
      const data = await response.json();
      setCourses(data);
      setCoursesCnt(data.length);
  ...
  const handleTypeWiseSearch = async (category,type) => {
    console.log(type, category)
    setType(type);
    try {
      const response = await fetch(
        `http://localhost:5002/courses/category/search/${category}/${type}`
      );
      const data = await response.json();
      setCourses(data);
      setCoursesCnt(data.length);
  ...
  //     try {
  //       const response = await fetch(
  //         `http://localhost:5002/courses?page=${page}&pageSize=${pageSize}`
  //       );
  //       const data = await response.json();
  //       setCourses(data);
  //       console.log(totalPages);
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        console.error("Error fetching courses:", error);
        // Optionally, set courses to an empty array in case of error
        setCourses([]);
      }
    };
    console.log("hello");
  ...
  return (
    <div className="bg-emerald-950 pb-40">
      <div className="w-[86%] m-auto">
        ...
        <section className=" pb-40 pt-20">
          <div className=" text-black flex justify-between gap-40">
            <div className="text-white ">
              ...
              <p>
                Interactive LIVE & Self-Paced Courses with Individual Attention
                by Industry Leading Gurus to Encourage Out-of-the-box thinking,
                leading to Clarity in Concepts, Creativity and Innovative Ideas.
              </p>
            </div>
            {/* input  */}
        ...
        <section className="text-2xl py-5 flex items-center gap-3">
          {
            options.map(option => 
              <div className="badge badge-accent text-xl p-4" onClick={()=>handleCategoryWiseSearch(option)}>{option}</div>
            )
          }
        </section>
        <section className="bg-neutral-950 p-10 rounded-2xl shadow-2xl">
          <div className="flex items-center justify-between mb-10">
            <div className="text-2xl text-white">
              All Courses ( {coursesCnt} )
            </div>
            <div>
            <form class="max-w-sm mx-auto">
            <label for="countries" class=" text-sm font-medium text-white dark:text-white">Type</label>
            {/* <select  onChange={(event) => 
              handleTypeWiseSearch(category, event.target.value)
             } 
            ...
            handleTypeWiseSearch= {handleTypeWiseSearch}
            category={category}
            />
          </form>
            </div>
          ...
          <div className="flex justify-center items-center bg-neutral-950 w-[38%] p-3 m-auto rounded-lg mt-20">
            <button
              className="btn btn-accent mr-10 w-[150px]"
              disabled={page === 1}
              onClick={() => setPage(page - 1)}
            >
              Previous Page
            </button>
            <span className="text-md text-white decoration-dashed">
              Page {page} of {totalPages}
            </span>
            <button
              className="btn btn-accent ml-10 w-[150px]"
