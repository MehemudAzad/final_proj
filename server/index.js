...
async function run() {
  try {
    ...
    app.post("/blog_comments", async (req, res) => {
      console.log("okay");
      try {
        const { blog_id, user_id, comment_text, date } = req.body;
        const newComment = await pool.query(
          "INSERT INTO blog_comments (blog_id, user_id, comment_text, comment_date) VALUES($1, $2, $3,$4) RETURNING *",
          [blog_id, user_id, comment_text,date]
        );
    ...
    app.post("/blog_comments_child/", async (req, res) => {
      try {
        ...
        console.log("Parent comment id of the blog is : ",parent_comment_id);
        res.json(newComment.rows[0]);
      } catch (err) {
        console.error(err.message);
      }
